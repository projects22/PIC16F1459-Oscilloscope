...
{   
    ...
    //ADC settings
    ADCON0=0b101001;    //0b101001 ADC=on, input=AN10 (pin 13)
    ADCON1=0B1010000;    //left just,tad 1/16,Vref=Vdd  tad 1/32 0B1000000 
    ADCON2=0x00;
        //comperator settings
    CM1CON0 = 0b11010110;      //comp on, invert, speed, hyst, 
    CM1CON1 = 0b10010010;       //interr, input+ DAC, input- pin 14  
    DACCON0 = 0b10000000;      //en, Vdd
    DACCON1 = 0b10000;          //2.5V
}
...
//            putUSBUSART(writeBuffer,numBytesRead);
            ...
            C1IF = 0;
            while(!C1IF){};     //wait for positive input
            C1IF = 0;
            
            for(i=0;i<61;++i){      //62 samples of input
                switch(timebase)
               {
                   case 49:   break; 
                   case 50:  __delay_us(6); break;
                   case 51:  __delay_us(600); break; 
                   case 52:  __delay_ms(10); break;
                   default: break;
               }                           
               
               GO=1;        //start ADC
               while(GO){}; //wait for end of conversion
               writeBuffer[i]=ADRESH/2;
            }
            putUSBUSART(writeBuffer,62);
        }
    }
